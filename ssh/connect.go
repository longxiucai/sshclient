...
import (
	...
	"github.com/longxiucai/sshclient/ssh/utils/hash"
...
func (s *SSHClient) connect(host net.IP) (*ssh.Client, error) {
	if s.Encrypted {
...
func (s *SSHClient) Connect(host net.IP) (*ssh.Client, *ssh.Session, error) {
...
func (s *SSHClient) sshAuthMethod(password, pkFile, pkPasswd string) (auth []ssh.AuthMethod) {
	if fileExist(pkFile) {
...
func (s *SSHClient) sshPrivateKeyMethod(pkFile, pkPassword string) (am ssh.AuthMethod, err error) {
...
func (s *SSHClient) sshPasswordMethod(password string) ssh.AuthMethod {
	return ssh.Password(password)
}

func (s *SSHClient) NewSudoSftpClient(conn *ssh.Client, opts ...sftp.ClientOption) (*sftp.Client, error) {
	var (
		cmd            string
		err            error
